...
def init(label, test_size = 0.2, train_size = 0.8, train = 'aps_failure_training_set_processed_8bit.csv', test = 'aps_failure_test_set_processed_8bit.csv', split = False):
    if test_size + train_size != 1.0:
        raise Exception(f'test_size and train_size must add up to 1.0, but got {train_size + test_size} instead.')
    ...
    if split:
        ...
        X_train, y_train, X_test, y_test = train_test_split(X, y, train_size = train_size, test_size = test_size, random_state = 101)
...
def hoeffding(X_train, y_train, max_samples, n_samples, classification = False, regression = False):
    if classification == False and regression == False:
        raise Exception('No mode selected, please set classification or regression to true.')
